...
function HomeGuideSection() {
  const { language } = useLanguage();
  const isArabic = language === "ar";
  const guide = homeGuideByLanguage[language as HomeLanguage];

  return (
    <section id="guide" className="relative px-4 py-12 sm:px-8 sm:py-16 lg:px-12">
      <div className="mx-auto max-w-6xl">
        <StaggerReveal amount={0.16}>
          <FadeUp>
            <div
              dir={isArabic ? "rtl" : "ltr"}
              className="overflow-hidden rounded-[30px] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.22)] sm:p-8"
            >
              <div className="flex flex-wrap gap-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-white/48">
                <span className="rounded-full border border-[#2BB673]/20 bg-[#2BB673]/10 px-3 py-1.5 text-[#8EE8B6]">
                  {guide.bylineLabel}: {guide.bylineValue}
                </span>
                <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5">
                  {guide.languageLabel}: {guide.languageValue}
                </span>
                <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5">
                  {guide.reviewedLabel}: {guide.reviewedValue}
                </span>
              </div>

              <h2 className="mt-5 max-w-3xl text-[30px] font-semibold leading-tight tracking-[-0.03em] text-[#F5F7F4] sm:text-[40px]">
                {guide.title}
              </h2>
              ...
              <div className="mt-8 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
                <div className="overflow-hidden rounded-[24px] border border-white/8 bg-black/16">
                  <div className="border-b border-white/8 px-5 py-4 text-[12px] font-semibold uppercase tracking-[0.18em] text-[#8EE8B6]">
                    {guide.tableTitle}
                  </div>
                  <div className="overflow-x-auto">
                    <table className="min-w-full text-start">
                      <thead className="bg-white/[0.03] text-[11px] uppercase tracking-[0.18em] text-white/42">
                        <tr>
                          <th className="px-5 py-3 font-semibold">
                            {guide.columns.area}
                          </th>
                          <th className="px-5 py-3 font-semibold">
                            {guide.columns.details}
                          </th>
                          <th className="px-5 py-3 font-semibold">
                            {guide.columns.route}
                          </th>
                      ...
                      <tbody>
                        {guide.rows.map((row) => (
                          <tr
                            key={row.area}
                            className="border-t border-white/8 text-[14px] text-white/72"
                          >
                            ...
                            <td className="px-5 py-4">
                              <Link
                                href={row.href}
                                className="inline-flex rounded-full border border-[#2BB673]/20 bg-[#2BB673]/10 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#8EE8B6] transition hover:bg-[#2BB673]/14"
                              >
                                {row.routeLabel}
                              </Link>
                            </td>
                ...
                <div className="rounded-[24px] border border-white/8 bg-black/16 p-5 sm:p-6">
